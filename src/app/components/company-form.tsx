...
export default function CompanyForm({ onSubmit }: Readonly<CompanyFormProps>) {
  ...
  const handleSubmit = async (values: CompanyFieldValues) => {
    if (categories && countries) {
      await mutateAsync({
        ...values,
        categoryTitle:
          categories.find(({ id }) => id === values.categoryId)?.attributes
            .title ?? "",
        countryTitle:
          countries.find(({ id }) => id === values.countryId)?.attributes
            .title ?? "",
      });
    }
